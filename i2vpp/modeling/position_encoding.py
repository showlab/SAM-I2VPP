...
def apply_rotary_enc_qk(
    ...
    if repeat_freqs_k:
        r = xk_.shape[-2] // (64*64)
        if freqs_cis_k.is_cuda:
            freqs_cis_k = freqs_cis_k.repeat(*([1] * (freqs_cis_k.ndim - 2)), r, 1)
        else:
            # torch.repeat on complex numbers may not be supported on non-CUDA devices
            # (freqs_cis has 4 dims and we repeat on dim 2) so we use expand + flatten
            freqs_cis_k = freqs_cis_k.unsqueeze(2).expand(-1, -1, r, -1, -1).flatten(2, 3)
    xk_out = torch.view_as_real(xk_ * freqs_cis_k).flatten(3)
    return xq_out.type_as(xq).to(xq.device), xk_out.type_as(xk).to(xk.device)


def apply_rotary_enc_qk_st_fusion(
    xq: torch.Tensor,
    xk: torch.Tensor,
    freqs_cis_q: torch.Tensor,
    freqs_cis_k: torch.Tensor,
...
def apply_rotary_enc_qk_dynamic(
    xq: torch.Tensor,
    xk: torch.Tensor,
    freqs_cis_q: torch.Tensor,
    freqs_cis_k: torch.Tensor,
    repeat_freqs_k: bool,
    resolution_frame: torch.Tensor,
):
    xq_ = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2))
    xk_ = (
        torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2))
    ...
    # repeat freqs along seq_len dim to match k seq_len
    if repeat_freqs_k:
        r = xk_.shape[-2] // resolution_frame
        if freqs_cis_k.is_cuda:
            freqs_cis_k = freqs_cis_k.repeat(*([1] * (freqs_cis_k.ndim - 2)), r, 1)
        else:
            # torch.repeat on complex numbers may not be supported on non-CUDA devices
