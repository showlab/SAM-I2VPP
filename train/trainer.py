...
from utils.distributed import all_reduce_max, barrier, get_rank
...
class CheckpointConf:
    ...
    # if not None, train will be resumed from this checkpoint
...
class Trainer:
    """
    Trainer supporting the DDP train strategies.
    """
    ...
    def _save_checkpoint(self, checkpoint, checkpoint_path):
        ...
        entire train since usually only the last checkpoint is kept per run).
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        logging.info(f"Resuming train from {ckpt_path}")
    ...
    def run(self):
        ...
        if self.mode == "train":
            if self.epoch > 0:
                logging.info(f"Resuming train from epoch: {self.epoch}")
    ...
    def train_epoch(self, train_loader):
        ...
        progress = ProgressMeter(
            ...
            prefix="Train Epoch: [{}/{}]".format(self.epoch, self.max_epochs),
        )
        ...
        for data_iter, batch in tqdm(enumerate(train_loader), total=len(train_loader), desc="Train"):
            ...
            try:
                flag = self._run_step(batch, phase, loss_mts, extra_loss_mts)
                if not flag:
                    continue
                ...
                if self.where < 1.0:
                    ...
                    logging.warning(
                        f"Skipping scheduler update since the train is at the end, i.e, {self.where} of [0,1]."
                    )
    ...
    def _run_step(
        ...
        if not math.isfinite(loss.item()):
            error_msg = f"Loss is {loss.item()}, attempting to stop train"
