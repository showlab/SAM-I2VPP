...
def load_checkpoint_and_apply_kernels(
    ...
    if ckpt_state_dict_keys is not None:
    ...
    return pre_train_dict, checkpoint_path
...
def load_state_dict_into_model(
    ...
    Args:
        state_dict_sam1: A dictionary containing the model's
    ...
    flag = device == torch.device('cuda:0')
    ...
    if state_dict is not None:
        ...
        _, _ = model.load_state_dict(state_dict, strict=True)
        if flag:
            print("-------- Resuming Weights --------")

        return model

    state_dict_sam1, path_sam1 = state_dict_sam1

    # 1. Apply kernels
    if checkpoint_kernels is not None:
        for f in checkpoint_kernels:
            state_dict_sam1 = f(state_dict=state_dict_sam1)

    # 2. load sam1
    ### (1) image encoder
    image_encoder_checkpoint = {}
    for k, v in state_dict_sam1.items():
        if k.startswith("image_encoder."):
            new_key = k.replace("image_encoder.", "")
            image_encoder_checkpoint[new_key] = v

    missing_keys, unexpected_keys = model.image_encoder.trunk_image.load_state_dict(image_encoder_checkpoint, strict=False)

    if flag:
        if missing_keys:
            print("Missing keys when loading ----image_encoder---- weights (formatted):")
            print(json.dumps(missing_keys, indent=4))
        else:
            print("No missing keys when loading ----image_encoder---- weights.")
        if unexpected_keys:
            print("Unexpected keys when loading ----image_encoder---- weights (formatted):")
            print(json.dumps(unexpected_keys, indent=4))
        else:
            print("No unexpected keys when loading ----image_encoder---- weights.")
        print(f"****** Loaded ---- sam1 image_encoder---- weights from {path_sam1} successfully ! ******")

    ### (2) prompt encoder
    prompt_encoder_checkpoint = {}
    for k, v in state_dict_sam1.items():
        if k.startswith("prompt_encoder."):
            new_key = k.replace("prompt_encoder.", "")
            prompt_encoder_checkpoint[new_key] = v
    missing_keys, unexpected_keys = model.sam_prompt_encoder.load_state_dict(prompt_encoder_checkpoint, strict=False)
    if flag:
        if missing_keys:
            print("Missing keys when loading ----prompt_encoder---- weights (formatted):")
            print(json.dumps(missing_keys, indent=4))
        else:
            print("No missing keys when loading ----prompt_encoder---- weights.")
        if unexpected_keys:
            print("Unexpected keys when loading ----prompt_encoder---- weights (formatted):")
            print(json.dumps(unexpected_keys, indent=4))
        else:
            print("No unexpected keys when loading ----prompt_encoder---- weights.")
        print(f"****** Loaded ----sam 1 prompt_encoder---- weights from {path_sam1} successfully ! ******")

    ### (3) mask decoder
    mask_decoder_checkpoint = {}
    for k, v in state_dict_sam1.items():
        if k.startswith("mask_decoder."):
            new_key = k.replace("mask_decoder.", "")
            mask_decoder_checkpoint[new_key] = v
    # Create a new dictionary to store the updated key-value pairs
    updated_checkpoint = {}
    for key in mask_decoder_checkpoint.keys():
        if '.mlp.lin1.' in key:
            new_key = key.replace('.mlp.lin1.', '.mlp.layers.0.')
        elif '.mlp.lin2.' in key:
            new_key = key.replace('.mlp.lin2.', '.mlp.layers.1.')
        else:
            new_key = key  # For keys that do not require replacement, keep them unchanged
        # Add the updated key-value pairs to the new dictionary
        updated_checkpoint[new_key] = mask_decoder_checkpoint[key]
    # Replace the original dictionary with the updated one
    mask_decoder_checkpoint = updated_checkpoint
    missing_keys, unexpected_keys = model.sam_mask_decoder.load_state_dict(mask_decoder_checkpoint, strict=False)
    if flag:
        if missing_keys:
            print("Missing keys when loading ----mask_decoder---- weights (formatted):")
            print(json.dumps(missing_keys, indent=4))
        else:
            print("No missing keys when loading ----mask_decoder---- weights.")
        if unexpected_keys:
            print("Unexpected keys when loading ----mask_decoder---- weights (formatted):")
            print(json.dumps(unexpected_keys, indent=4))
        else:
            print("No unexpected keys when loading ----mask_decoder---- weights.")
        print(f"****** Loaded ----sam 1 mask_decoder---- weights from {path_sam1} successfully ! ******")
